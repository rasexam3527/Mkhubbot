...
async def records_callback(update, context):
    ...
    # Records are OWNER ONLY. Admins with demo/both access cannot see them.
    if not is_owner(uid):
        await q.answer("Owner only.", show_alert=True)
        return
    ...
    with db() as con:
        rows = con.execute(
            """
            SELECT admin_name,channel_name,link_type,link_url,created_at
            FROM records
            ORDER BY id DESC
            LIMIT 20
            """
    ...
    if rows:
        lines.append("<b>Generated Links</b>")
        for r in rows:
            icon = DEMO if r["link_type"] == "DEMO" else PERM
            lines.append(
                f"{icon} <b>{escape(repair_mojibake(r['channel_name']) or 'Course')}</b>\n"
                f"{USER} {escape(r['admin_name'] or 'Admin')}\n"
                f"Time: {escape(r['created_at'] or '-')}\n"
            )
    else:
        lines.append("No generated links yet.")

    lines.append("\n<b>Demo Members</b>")
    if members:
        for m in members:
            display_name = m["member_name"] or "Unknown"
            username = f"@{m['username']}" if m["username"] else "No username"
            joined = m["joined_at"] or "-"
            status = m["status"] or "active"
            lines.append(
                f"{USER} <b>{escape(display_name)}</b> ({escape(username)})\n"
                f"ID: <code>{m['user_id']}</code>\n"
                f"{BOOK} {escape(repair_mojibake(m['channel_name']) or 'Course')}\n"
                f"Joined: {escape(joined)} | Status: <b>{escape(status)}</b>\n"
            )
    else:
        lines.append("No demo member joined yet.")
